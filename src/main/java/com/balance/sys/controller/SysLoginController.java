...
public class SysLoginController {
	...
	public void checkLogin(HttpServletRequest request, HttpServletResponse response, String username, String password, String verifyCode) {
		...
		if (StringUtil.isNullOrEmpty(username) || StringUtil.isNullOrEmpty(password)) {
			WebUtil.out(response, JsonUtil.createOperaStr(false, "军官证号或密码错误"));
			return;
		}

		SysUser sysUser = sysUserService.getByUsername(username);
		if (sysUser == null || !sysUserService.checkPass(sysUser, password)) {
			String json = "{\"success\":" + false + ",\"msgText\":\"军官证号或密码错误\"}";
			...
			if (sysUser.getStatus() == 2 || sysUser.getStatus() == 9) {
				...
				// 如果是学员，查询class_id
				if (sysUser.getType() == 4) {
					userSession.setClass_id(baseUserInfoService.getClass_id(userSession.getUser_id()));
				} else {
					userSession.setClass_id(0);
				}
